...
function cardHTML(cat, d, i) {
  ...
  return '<div class="' + cardClass + '">' + badge + '<div class="dish-image' + noimgClass + '" ' + imgStyle + ' ' + clickFoto + '>' + icono + placeholder + '</div><div class="dish-content"><div class="dish-name">' + d.name + '</div><div class="dish-description">' + d.desc + '</div>' + ratingHtml + priceHtml + '<div class="dish-buttons"><div class="btn-group"><button class="btn-cart btn-cart-animate" ' + btnDisabled + ' onclick="addToCartWithNotes(\'' + safeProductName + '\', ' + d.price + ', \'' + cat + '\', \'' + (d.icon || icono || '🍽️').replace(/'/g, "\\'") + '\')">' + btnText + '</button></div></div><div class="dish-actions" style="display:none"><button class="edit-btn" onclick="editItem(\'' + safeCat + '\',' + i + ')">✏️ Editar</button><button class="delete-btn" onclick="deleteItem(\'' + safeCat + '\',' + i + ')">🗑️ Borrar</button></div></div></div>';
}
